...
#         if data_type == "xml":
#             with open(filename, "r", encoding="utf-8") as f:
#                 root = xmltodict.parse(f.read(), attr_prefix="", dict_constructor=dict)
#         else:
#             with open(filename, "r", encoding="utf-8") as f:
#                 root = json.load(f)
...
#         root = replace_nil_true(root)
#         root = wrap_dicts_as_lists(root)
#         root = get_nested_dict(root, feature_set.ROOT)
...
#                 data_type=data_type,
#             )
#             processor.process(output_format=output_format)
...
#             # 🔑 map future -> filename
#             future_to_filename = {
#                 executor.submit(process_file, task): task[0]
#                 for task in tasks
#             }
...
#                 try:
#                     result = future.result()  # captures worker exceptions
...
#                 except Exception as e:
#                     # hard crash inside process_file
#                     print(f"errrr : {e}")
#                     failed += 1
#                     failed_files.append((filename, str(e)))
