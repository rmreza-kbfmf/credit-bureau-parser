...
class CIPReasonProcessor(BaseProcessor):
    @safe_run(use_logger=False)
    def process(self, output_format=None):
        self.sections = [
            (self.feature_set.CIP_ROOT, self.feature_set.CIP_REASON)
        ]
...
class CIPProcessor(BaseProcessor):
    ...
    def process(self, output_format=None):
        ...
        for section_key, fields in self.sections:
            ...
            # --- identification section (singleton)
